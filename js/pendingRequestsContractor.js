function loadPendingRequests(elementName){
    ...
    $.ajax({
        ...
        success: function (data) {
           ...
           $(elementName).html(userData.firstName); 
            ...
            $.ajax({
                url: 'http://localhost:8080/ContractorPendingRequests',
                data: userdata,
                type: 'POST',
                contentType: 'application/json',
                jsonpCallback: 'callback', // this is not relevant to the POST anymore
                success: function (data) {
                    ...
                    for(var i = 0 ; i < responseData.length;i++) {
                        $("#requestsTable").append(
                            ...
                            + "<td id =jobAddress" + i + ">"  + responseData[i].jobAddress   + "</td>"
                            + "<td> <button onclick = 'acceptJob(this.className)' class = " + i + "> Accept </button> <button onclick = 'declineJob(this.className)' class = " + i + "> Decline </button> </td>"
                            + "</tr>"
