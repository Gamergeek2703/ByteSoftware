...
function loadActiveRequests(elementName){
    ...
    $.ajax({
        ...
        success: function (data) {
           ...
           $(elementName).html(userData.firstName);
            ...
            $.ajax({
                url: 'http://localhost:8080/ContractorActiveRequests',
                data: userdata,
                type: 'POST',
                contentType: 'application/json',
                jsonpCallback: 'callback', // this is not relevant to the POST anymore
                success: function (data) {
                    ...
                    for(var i = 0 ; i < responseData.length;i++) {
                        $("#requestsTable").append(
                            ...
                            + "<td> <button onclick = 'completeJob(this.className)' class = " + i + "> Complete  </td>"
                            + "</tr>"
